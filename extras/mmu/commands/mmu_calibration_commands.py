...
class MmuCalibrateGearCommand(CalibrationMixin, BaseCommand):
    """
...
class MmuCalibrateEncoderCommand(CalibrationMixin, BaseCommand):
    """
        ...
        try:
            ...
            with mmu.wrap_sync_gear_to_extruder():
                with mmu.require_encoder():
                    ...
                    if measured < mmu_unit.encoder.movement_min():
...
class MmuCalibrateGateCommand(CalibrationMixin, BaseCommand):
    """
...
class MmuCalibrateBowdenCommand(CalibrationMixin, BaseCommand):
    """
    ...
    def _run(self, gcmd):
        ...
        can_use_sensor = (
            mmu_unit.p.extruder_homing_endstop in [
                ...
                sensor_manager.has_sensor(mmu_unit.p.extruder_homing_endstop) or
                mmu.drive(gate).is_endstop_virtual(mmu_unit.p.extruder_homing_endstop)
            )
        )
...
class MmuCalibrateToolheadCommand(CalibrationMixin, BaseCommand):
    """
    ...
    def _run(self, gcmd, mmu_unit):
        ...
        if not sensor_manager.has_sensor(SENSOR_TOOLHEAD):
