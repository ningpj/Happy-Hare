...
class SyncStateTest(object):
    ...
    def __str__(self):
        return "compression_sensor=%s, tension_sensor=%s, compression_state=%s, tension_state=%s, toggle_compression=%s, toggle_tension=%s, triggered_sensor=%s" % (
            self.compression_sensor, self.tension_sensor, self.compression_state, self.tension_state, self.toggle_compression, self.toggle_tension, self.triggered_sensor
        )
...
class MmuTestCommand(BaseCommand):
    """
    ...
    def _run(self, gcmd):
        ...
        try:
            ...
            if sync_state is not None:
                ...
                if sync_state == 'loop':
                    ...
                    def gather_state(state):
                        mmu.log_trace(" -- Gathered state: %s" % len(gathered_states))
                    ...
                    def display_results():
                        ...
                        mmu.log_debug("Configuration repartition")
                        nb_hits = {}
                        ...
                        for i, test in enumerate(tests):
                            ...
                            if test.expected != gathered_states[i]:
                                mmu.log_debug(
                                    "MISMATCH on test id:%s (expected : %s) -> %s" %
                                    (str(test.id), test.expected, gathered_states[i])
                                )
                                mmu.log_debug("   Test : %s" % (str(test)))
                                mismatches[str(test)]['count'] += 1

                        # Display mismatches
                        nb_mismatches = sum([v['count'] for v in mismatches.values()])
                        mmu.log_info(
                            "Total Mismatches: " + str(nb_mismatches) + '/' + str(len(tests)) +
                            ' (' + str(round(nb_mismatches / len(tests) * 100, 2)) + ' %)'
                        )

                        if mismatches:
                            # Sort by most mismatches (highest first)
                            mismatches = dict(sorted(mismatches.items(), key=lambda item: item[1]['count'], reverse=True))
                            for test_str, info in mismatches.items():
                                if info['count']:
                                    mmu.log_debug(
                                        "MISMATCH: %s (expected : %s) -> %s" %
                                        (test_str, info['test'].expected, info['count'])
                                    )

                            # Summary: which expected state has which percentage of total errors
                            for expected in [1, 0, -1]:
                                if nb_tests_by_expected[expected]:
                                    nb_err_count = sum([
                                        info['count'] for __, info in mismatches.items()
                                        if info['test'].expected == expected
                                    ])
                                    mmu.log_debug(
                                        ">>>>>> Expected state " + str(expected) + " -> " +
                                        str(nb_err_count) + '/' + str(nb_tests_by_expected[expected]) +
                                        ' (' + str(round(nb_err_count / nb_tests_by_expected[expected] * 100, 2)) + ' %)'
                                    )
                                    mmu.log_debug("   Edge detection error repartition")
                                    # compression rising edge
                                    count = sum([
                                        info['count'] for __, info in mismatches.items()
                                        if info['test'].expected == expected and info['test'].toggle_compression == 'rising edge'
                                    ])
                                    if count:
                                        mmu.log_debug(
                                            "      " + str(count) + '/' + str(nb_err_count) + ' (' +
                                            str(round(((count / nb_err_count) if nb_err_count else 0) * 100, 2)) +
                                            ' %) ' + 'compression rising edge'
                                        )
                                    # compression falling edge
                                    count = sum([
                                        info['count'] for __, info in mismatches.items()
                                        if info['test'].expected == expected and info['test'].toggle_compression == 'falling edge'
                                    ])
                                    if count:
                                        mmu.log_debug(
                                            "      " + str(count) + '/' + str(nb_err_count) + ' (' +
                                    ...
                                    count = sum([
                                        info['count'] for __, info in mismatches.items()
                                        if info['test'].expected == expected and info['test'].toggle_tension == 'rising edge'
                                    ])
                                    if count:
                                        mmu.log_debug(
                                            "      " + str(count) + '/' + str(nb_err_count) + ' (' +
                                    ...
                                    # tension falling edge
                                    count = sum([
                                        info['count'] for __, info in mismatches.items()
                                        if info['test'].expected == expected and info['test'].toggle_tension == 'falling edge'
                                    ])
                                    if count:
                                        mmu.log_debug(
                                            "      " + str(count) + '/' + str(nb_err_count) + ' (' +
                            ...
                            mmu.log_info("No mismatches")

                    mmu.printer.register_event_handler("mmu:sync_feedback_finished", gather_state)
                    mmu.printer.register_event_handler("mmu:test_gen_finished", wait_for_results)
                    ...
                    for sensor_scenario in ['compression_only', 'tension_only', 'both', 'none']:
                        ...
                        mmu.log_info(">>>>>> Testing sensor configuration '%s'" % sensor_scenario)
                        ...
                        while len(tests) < nb_iterations:
                            ...
                            # generate a random sync state (only one sensor changes at a time)
                            if random.choice([True, False]):
                                ...
                                if compression_sensor_filament_present != compression_test_sensor.runout_helper.filament_present:
                                    ...
                                    mmu.log_trace(" -- Generated test: %s" % len(tests))
                                    tests.append(SyncStateTest(
                                        compression_test_sensor, tension_test_sensor,
                                        compression_sensor_filament_present, tension_sensor_filament_present,
                                        toggle_compression, toggle_tension, triggered_sensor, len(tests)
                                    ))
                            else:
                        ...
                        mmu.log_trace(" -- All %d were generated" % len(tests))
                        mmu.printer.send_event("mmu:test_gen_finished")
                        wait_run()
                        gathered_states = []
                        tests = []
                    ...
                    mmu.log_info("See mmu.log for a more details")

                else:
                    if sync_state == 'compression':
                        if compression_test_sensor is not None:
                            mmu.log_info("Setting compression sensor to 'detected'")
                            compression_sensor_filament_present = True
                        if tension_test_sensor is not None:
                            mmu.log_info("Setting tension sensor to 'not detected'")
                            tension_sensor_filament_present = False
                    elif sync_state == 'tension':
                        if compression_test_sensor is not None:
                            mmu.log_info("Setting compression sensor to 'not detected'")
                            compression_sensor_filament_present = False
                        if tension_test_sensor is not None:
                            mmu.log_info("Setting tension sensor to 'detected'")
                        ...
                        mmu.log_error("Invalid sync state: %s" % sync_state)

                    # Generate a tension or compression event
                    mmu.log_trace(">>>>>> sync test Testing configuration %s" % (sync_state.upper()))
                    if compression_test_sensor is not None:
                        compression_test_sensor.runout_helper.note_filament_present(compression_sensor_filament_present)
                    if tension_test_sensor is not None:
                        tension_test_sensor.runout_helper.note_filament_present(tension_sensor_filament_present)
                ...
                with mmu._wrap_track_time('total'):
                    with mmu._wrap_track_time('unload'):
                        with mmu._wrap_track_time('pre_unload'):
                            mmu.wrap_gcode_command(mmu.p.pre_unload_macro, exception=False, wait=True)
                        mmu.wrap_gcode_command(mmu.p.post_form_tip_macro, exception=False, wait=True)
                        with mmu._wrap_track_time('post_unload'):
                            mmu.wrap_gcode_command(mmu.p.post_unload_macro, exception=False, wait=True)
                    with mmu._wrap_track_time('load'):
                        with mmu._wrap_track_time('pre_load'):
                            mmu.wrap_gcode_command(mmu.p.pre_load_macro, exception=False, wait=True)
                        with mmu._wrap_track_time('post_load'):
                            mmu.wrap_gcode_command(mmu.p.post_load_macro, exception=False, wait=False)
                            if error:
            ...
            if gcmd.get_int('RUN_CHANGE_SEQUENCE', 0, minval=0, maxval=1):
                ...
                goto_pos = None
                if next_pos == 'next':
                    mmu.wrap_gcode_command(
                        "SET_GCODE_VARIABLE MACRO=_MMU_SEQUENCE_VARS VARIABLE=restore_xy_pos VALUE='\"%s\"'" % next_pos
                    )
                    goto_pos = [11, 11]
                    ...
                    try:
                        ...
                        with mmu._wrap_track_time('load'):
                            ...
                            if pause:
                                ...
                                with mmu._wrap_track_time('post_load'):
                                    mmu.wrap_gcode_command(mmu.p.post_load_macro, exception=False, wait=True)
                                mmu._restore_toolhead_position('toolchange')
            ...
            if sync is not None:
                ...
                DRIVE_MODE_BY_NAME = {v.lower(): k for k, v in DRIVE_MODE_NAMES.items()}
                try:
                    mode = int(sync)
                except (TypeError, ValueError):
                    key = str(sync).lower()
                    mode = DRIVE_MODE_BY_NAME.get(key)
                    if mode is None:
                        valid = ", ".join(DRIVE_MODE_BY_NAME.keys())
                        raise gcmd.error(f"Invalid SYNC value: {sync}. Expected one of: {valid} or an integer.")
                    ...
                    for i in range(loop):
                        ...
                        for j in range(6):
                            ...
                            if servo and random.randint(0, 4) == 0:
                                ...
                                mmu.gcode.run_script_from_command("MMU_SERVO POS=%s" % pos)
                            ...
                            actual, _, _, _ = mmu.move_filament(
                                ...
                                encoder_dwell=encoder_dwell, speed_override=False, wait=wait
                            )
                            tracked += actual
                        ...
                        if not math.isclose(expected, tracked):
                            raise MmuError(
                                "TEST ERROR: inital_pos=%.6f, final_pos=%.6f, tracked=%.6f (expected=%.6f)" %
                                (initial_pos, final_pos, tracked, expected)
                            )
                        ...
                        for j in range(5):
                            ...
                            # Simulate callback into _MMU_STEP_MOVE / _MMU_STEP_HOMING_MOVE
                            if random.randint(0, 4) == 0:
                                log(">>> Calling _MMU_STEP_** move")
                                if random.randint(0, 1):
                                    mmu.gcode.run_script_from_command("_MMU_STEP_MOVE MOVE=10")
                                else:
                                    mmu.gcode.run_script_from_command("_MMU_STEP_HOMING_MOVE MOVE=10 ENDSTOP=%s" % endstop)
            ...
            if gcmd.get_int('SYNC_LOAD_TEST', 0, minval=0, maxval=1):
                ...
                for i in range(loop):
                    ...
                    if move_type in (0, 1):
                        ...
                        mmu.movequeue_wait()
                        tracking = abs(mmu.drive().get_filament_position() - total) < 0.1
                        mmu.log_info(">>>>>> STATUS: sync: %s, pos=%.2f, total=%.2f" % (sync, mmu.drive().get_filament_position(), total))
                        if not tracking:
            ...
            if gcmd.get_int('TTC_TEST', 0, minval=0, maxval=1):
                ...
                for i in range(loop):
                    ...
                    mmu.gcode.run_script_from_command(
                        "MMU_TEST_HOMING_MOVE MOTOR=%s MOVE=5 ENDSTOP=toolhead STOP_ON_ENDSTOP=%d DEBUG=%d" %
                        (motor, stop_on_endstop, debug)
                    )
                    if random.randint(0, 1):
                        mmu.gcode.run_script_from_command(
            ...
            if gcmd.get_int('TTC_TEST2', 0, minval=0, maxval=1):
                have_run_test = True
                loop = gcmd.get_int('LOOP', 10, minval=1, maxval=10000000)
                debug = gcmd.get_int('DEBUG', 0, minval=0, maxval=1)
                mix = gcmd.get_int('MIX', 0, minval=0, maxval=1)
                wait = gcmd.get_int('WAIT', None, minval=0, maxval=1)
                for i in range(loop):
            ...
            if gcmd.get_int('TTC_TEST3', 0, minval=0, maxval=1):
                ...
                for i in range(loop):
                    ...
                    motor = "gear"
                    mmu.gcode.run_script_from_command(
                        "MMU_TEST_HOMING_MOVE MOTOR=%s MOVE=-70 SPEED=300 ACCEL=1000 ENDSTOP=toolhead STOP_ON_ENDSTOP=%d DEBUG=%d" %
                        (motor, stop_on_endstop, debug)
                    )
                    if random.randint(0, 1):
                        mmu.gcode.run_script_from_command(
            ...
            if gcmd.get_int('CALC_PURGE', 0, minval=0, maxval=1):
                ...
                mmu.log_always("The purge vol from RGB color 192, 192, 192 to 247, 35, 35 is: {}".format(purge_vol))
            ...
            if gcmd.get_int('SENSOR', 0, minval=0, maxval=1):
                ...
                if not loop:
                    sensors = mmu.sensor_manager.get_sensors_before(pos, gate, loading=loading)
                    mmu.log_always("check_all_sensors_before(%s,%s)=%s" % (pos, gate, mmu.sensor_manager.check_all_sensors_before(pos, gate, loading=loading)))
                    mmu.log_always("sensors before=%s" % sensors)

                    sensors = mmu.sensor_manager.get_sensors_after(pos, gate, loading=loading)
                    mmu.log_always("check_all_sensors_after(%s,%s)=%s" % (pos, gate, mmu.sensor_manager.check_all_sensors_after(pos, gate, loading=loading)))
                    mmu.log_always("sensors after=%s" % sensors)
                else:
                    for pos_i in range(-1, 11):
                        mmu.log_always("check_all_sensors_before(%s,%s)=%s" % (pos_i, gate, mmu.sensor_manager.check_all_sensors_before(pos_i, gate, loading=loading)))
                    mmu.log_always("")
                    for pos_i in range(-1, 11):
                        mmu.log_always("check_all_sensors_after(%s,%s)=%s" % (pos_i, gate, mmu.sensor_manager.check_all_sensors_after(pos_i, gate, loading=loading)))
            ...
            mmu._is_running_test = False  # Restore non testing context
